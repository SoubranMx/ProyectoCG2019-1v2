...
void CFiguras::aulaI(GLuint text[4], float largo, float altura, float profundidad) {
	//texturas:
	/*
		0	Pared
		1	Ventana
	...
	*/
	...
	glBindTexture(GL_TEXTURE_2D, text[0]);   // choose the texture to use.
	glBegin(GL_POLYGON);	//Front		Pizarron
		//glColor3f(0.0,0.0,1.0);
		glNormal3f(0.0f, 0.0f, -1.0f);	//la normal para adentro, porque es el laboratorio.
		glTexCoord2f(0.0f, 0.0f); glVertex3fv(vertice[0]);
	...
	glBegin(GL_POLYGON);	//Right		Ventanas
		glNormal3f(-1.0f, 0.0f, 0.0f);
		glTexCoord2f(0.0f, 0.0f); glVertex3fv(vertice[0]);
		glTexCoord2f(1.0f, 0.0f); glVertex3fv(vertice[3]);
		glTexCoord2f(1.0f, 1.0f); glVertex3fv(vertice[5]);
